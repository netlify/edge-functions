// Source: https://github.com/denoland/deno_std/blob/main/datetime/formatter.ts

// Copyright 2018-2022 the Deno authors. All rights reserved. MIT license.
// This module is browser compatible.

import { CallbackResult, ReceiverResult, Rule, TestFunction, TestResult, Tokenizer } from './tokenizer'
...
function createLiteralTestFunction(value: string): TestFunction {
  return (string: string): TestResult => {
    return string.startsWith(value) ? { value, length: value.length } : undefined
  }
}
...
export class DateTimeFormatter {
  ...
  parseToParts(string: string): DateTimeFormatPart[] {
    ...
    for (const token of this.#format) {
      ...
      switch (token.type) {
        ...
        case 'fractionalSecond': {
          value = new RegExp(`^\\d{${token.value}}`).exec(string)?.[0] as string
          break
        }
        case 'timeZoneName': {
  ...
  sortDateTimeFormatPart(parts: DateTimeFormatPart[]): DateTimeFormatPart[] {
    ...
    const typeArray = ['year', 'month', 'day', 'hour', 'minute', 'second', 'fractionalSecond']
    ...
    for (const part of parts) {
      switch (part.type) {
        ...
        case 'hour': {
          ...
          const dayPeriod = parts.find((part: DateTimeFormatPart) => part.type === 'dayPeriod')
